...
document — usable by **both** the `scanoss` CLI and external consumers (e.g. Earnie)
...
result; Earnie assembles it from its stored identifications (a user clicks "export →
...
- Licenses and vulnerabilities come from **decoration services**, not the scan payload:
  ...
  *openapi.VulnerabilitiesResponse`. Both return structured, already-discrete data (no
  ...
  are **declared**, while **concluded** licenses come only from Earnie's identifications.
...
- **`pkg/sbom`** depends only on `cyclonedx-go`. Earnie can import it without pulling in
...
- **`pkg/sbom/scansource`** holds the only SDK-coupled code: adapters that build an
  ...
  `*openapi.VulnerabilitiesResponse`. The CLI uses it; Earnie does not.
- One generator, two ways of assembling the inventory.

SDK usage (Earnie — no scan, no network at report time):
```go
inv := buildInventoryFromIdentifications()  // Earnie's stored components + vulnerabilities
...
const (
    ...
    AckConcluded LicenseAcknowledgement = "concluded" // determined by review (Earnie)
)
...
- There is exactly **one** generation function. The per-format builders
  ...
  format at runtime (a CLI flag, an Earnie button), so a typed format + one dispatcher
...
### Primary user story
As an SBOM consumer (the CLI, or Earnie via the SDK) I assemble an inventory of the
...
7. **Given** the CLI `scanoss scan ./proj -f cyclonedx`, **then** it builds the
   ...
   call).
8. **Given** an SDK consumer (Earnie) with stored identifications and **no** live scan,
...
- **FR-007 (licenses + vulnerabilities via decoration → inventory)** Licenses and
  ...
  - Concluded licenses are not produced by `scansource` — they come only from a consumer
    (Earnie) setting `Component.Licenses` with `AckConcluded` entries.
...
0. **Licenses sourced from the decoration service** (`client.Licenses.Components`), not
   ...
   only from a consumer (Earnie). The scan payload's `licenses` field is no longer read.
...
7. **Does the CLI keep generating at scan time?** Per "generate from identifications, not
   ...
   just assembles its inventory differently from Earnie). _Confirm, or restrict SBOM
...
- **Exposing `pkg/sbom` through the `libscanoss` (Python/Node) bindings.** If Earnie is
