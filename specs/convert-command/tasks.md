...
- [x] **T1 — CycloneDX reader.** `pkg/sbom/parse_cyclonedx.go`:
...
- [x] **T2 — SPDX reader.** `pkg/sbom/parse_spdx.go`:
...
- [x] **T3 — Round-trip tests.** In `pkg/sbom`: `Generate → Parse → Inventory` equality for
  both formats, over a representative `Inventory` (components, licenses, vulnerabilities).

- [x] **T4 — `convert` command.** `cmd/convert.go`: offline `convert <input> --format
...
- [x] **T5 — Lossy-conversion warnings.** Warn once per layer the target can't represent
  (e.g. `cyclonedx → spdx` drops vulnerabilities). (FR-006)

- [x] **T6 — Command tests.** `cmd/convert_test.go`: `identifyFormat` table; end-to-end each
  direction (spdx↔cyclonedx, raw→each); vulnerability-dropped warning; invalid `--format`.

- [x] **T7 — Docs + changelog.** Document the new command:
